...
foreach($users as $u){
    $uid=$u->id;
    $name=$u->name;
    $presence_count_pre_emp = DB::table('daily_attendances')->whereYear('date','=',"2020-01-10")->whereMonth('date','=',"2020-01-10")->where('emp_id', '=', $uid )->where('status', '=', 'presence' )->count(); 
    
    array_push($this_month_summary,array($name,$presence_count_pre_emp));
}

$sumMonth= json_encode($this_month_summary);
           return view('daily_attendance.progress',compact('sumYear','sumMonth'));        
        }
